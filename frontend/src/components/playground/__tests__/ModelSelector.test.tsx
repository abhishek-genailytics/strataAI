...
describe("ModelSelector", () => {
  ...
  it("renders model options after loading", async () => {
    ...
    expect(
      screen.getByText("🧠 Claude 3 Opus (anthropic)")
    ).toBeInTheDocument();
  });
  ...
  it("shows model details when a model is selected", async () => {
    ...
    expect(
      screen.getByText("Cost: $0.03/1K input • $0.06/1K output")
    ).toBeInTheDocument();
  ...
  it("shows no models message when no models are available", async () => {
    mockApiService.getModels.mockResolvedValue({ data: [] });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          "No models available. Please configure access tokens in the Access section."
        )
      ).toBeInTheDocument();
    });
  });
