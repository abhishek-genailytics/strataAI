            ...
            <div className="rounded-md border">
              <ScrollArea className="h-56">
                <div className="divide-y">
                  ...
                  ) : (models?.length ? models.map((m: ModelInfo) => (
                    <label key={m.id} className="flex items-center gap-3 p-3 cursor-pointer">
                      <Checkbox checked={!!selected[m.id]} onCheckedChange={(v: boolean)=>setSelected(s=>({ ...s, [m.id]: !!v }))} />
                  ...
                  )) : <div className="p-3 text-sm text-slate-500">No models found for this provider.</div>)}
                </div>
              </ScrollArea>
            </div>
          </div>
