...
const metricData = [
  { time: "08:00", "gpt-4o": 0, "claude-3-sonnet": 0, "gpt-4": 0, "gemini-2": 0, "gpt-3.5": 0 },
  { time: "08:30", "gpt-4o": 150000, "claude-3-sonnet": 80000, "gpt-4": 45000, "gemini-2": 120000, "gpt-3.5": 200000 },
  { time: "09:00", "gpt-4o": 320000, "claude-3-sonnet": 180000, "gpt-4": 90000, "gemini-2": 250000, "gpt-3.5": 450000 },
  { time: "09:30", "gpt-4o": 780000, "claude-3-sonnet": 420000, "gpt-4": 180000, "gemini-2": 580000, "gpt-3.5": 890000 },
  { time: "10:00", "gpt-4o": 1200000, "claude-3-sonnet": 650000, "gpt-4": 280000, "gemini-2": 850000, "gpt-3.5": 1100000 },
];

const outputData = [
  { time: "08:00", "gpt-4o": 0, "claude-3-sonnet": 0, "gpt-4": 0, "gemini-2": 0, "gpt-3.5": 0 },
  { time: "08:30", "gpt-4o": 2500, "claude-3-sonnet": 1800, "gpt-4": 1200, "gemini-2": 2000, "gpt-3.5": 3200 },
  { time: "09:00", "gpt-4o": 5800, "claude-3-sonnet": 4200, "gpt-4": 2800, "gemini-2": 4800, "gpt-3.5": 7200 },
  { time: "09:30", "gpt-4o": 8900, "claude-3-sonnet": 6500, "gpt-4": 4200, "gemini-2": 7200, "gpt-3.5": 10500 },
  { time: "10:00", "gpt-4o": 12139, "claude-3-sonnet": 8500, "gpt-4": 5800, "gemini-2": 9200, "gpt-3.5": 13800 },
];
...
export default function Monitor() {
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-slate-900">
                Request Latency
                <span className="text-sm font-normal text-slate-500 ml-2">(milliseconds)</span>
              </h3>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg xl:col-span-2">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-slate-900">
                Input Tokens
                <span className="text-sm font-normal text-slate-500 ml-2">(tokens)</span>
              </h3>
              <div className="flex items-center space-x-2">
                <Button variant="ghost" size="sm" className="text-slate-400 hover:text-slate-600">
                  <Code className="w-4 h-4" />
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={metricData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                />
                {Object.entries(modelColors).slice(0, 5).map(([model, color]) => (
                  <Bar
                    key={model}
                    dataKey={model}
                    fill={color}
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg xl:col-span-2">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={outputData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                <XAxis 
                  dataKey="time" 
                  tick={{ fontSize: 12, fill: '#64748b' }}
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-slate-900">
                Requests Per Second
                <span className="text-sm font-normal text-slate-500 ml-2">(requests/second)</span>
              </h3>
            </div>
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-slate-900">Model Performance Summary</h3>
            <div className="flex items-center space-x-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {[
              { name: "gpt-4o", requests: "45.2K", latency: "1.2s", cost: "$2.85", status: "healthy" },
              { name: "claude-3-sonnet", requests: "32.1K", latency: "0.9s", cost: "$1.95", status: "healthy" },
              { name: "gpt-4", requests: "18.5K", latency: "1.8s", cost: "$4.20", status: "warning" },
              { name: "gemini-2", requests: "28.7K", latency: "1.1s", cost: "$1.65", status: "healthy" },
              { name: "gpt-3.5", requests: "67.3K", latency: "0.6s", cost: "$0.85", status: "healthy" },
            ].map((model) => (
              <div key={model.name} className="p-4 border border-slate-200 rounded-xl hover:border-slate-300 transition-colors bg-white/60">
                <div className="flex items-center justify-between mb-3">
                  <span className="font-semibold text-slate-900 text-sm">{model.name}</span>
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-900">{model.requests}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-900">{model.cost}</span>
                  </div>
                </div>
              </div>
            ))}
