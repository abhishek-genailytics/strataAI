...
export const OrganizationProvider = ({
    ...
    const loadOrganizations = async () => {
      try {
        ...
        if (!session?.access_token) {
          console.warn(
            "No active Supabase session, skipping organization loading"
          );
          return;
        }
        ...
        // Since each user has only one organization, use the first (and only) one
        const userOrg = organizations[0] || null;
        setCurrent(userOrg);

        if (userOrg) {
          localStorage.setItem("org_id", userOrg.id);
          console.log("Organization context set:", userOrg.name, userOrg.id);
        } else {
  ...
  return (
    <OrgContext.Provider value={{ current, orgs, setCurrent }}>
      {children}
    </OrgContext.Provider>
  );
};
